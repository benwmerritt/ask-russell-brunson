#!/usr/bin/env python3
"""
Remote MCP Server for Ask Russell Brunson

Production-ready remote MCP server that exposes Russell Brunson's sales funnel
and online business content to LLM applications like Claude Web/iOS via API key authentication.
...
# All tools are already registered there (ask_russell_brunson, about)
mcp = mcp_server.mcp
...
app = FastAPI(
    title="Ask Russell Brunson Remote MCP",
    version="1.0.0",
    description="Remote Model Context Protocol server for Russell Brunson's sales funnel and business content",
...
async def health():
    ...
    return {
        "ok": True,
        "service": "Ask Russell Brunson Remote MCP",
        "version": "1.0.0",
...
async def root():
    ...
    return {
        "name": "Ask Russell Brunson Remote MCP",
        "version": "1.0.0",
        "description": "Remote MCP server for Russell Brunson's sales funnel and business growth content",
        "topic": "Sales funnels, online business growth, and marketing strategies",
        "endpoints": {
            "mcp": f"{base_url}/mcp",
            "health": f"{base_url}/health",
        },
        "tools": ["ask_russell_brunson", "about"],
